...
void load()
{
	...
	qBert->GetComponent<dae::SpriteRenderer>()->SetSpriteSheet("QBert_JumpRightUp.png", 1, 2);
	qBert->GetComponent<dae::SpriteRenderer>()->SetFrameDelay(0.3f);
	qBert->GetComponent<dae::SpriteRenderer>()->PlayAnim();
	...
	input.GetInstance().AddAction("MoveUp", SDL_SCANCODE_W, dae::TriggerEvent::Down, std::make_unique<dae::MoveCommand>(*qBert.get(), glm::vec2{ 1,-5 }, 190.f));
	input.GetInstance().AddAction("MoveDown", SDL_SCANCODE_S, dae::TriggerEvent::Down, std::make_unique<dae::MoveCommand>(*qBert.get(), glm::vec2{ -1,-2.5 }, 90.f));
