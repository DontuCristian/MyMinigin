...
void dae::Renderer::RenderTexture(const Texture2D& texture, const SDL_Rect& dst, const SDL_Rect& src, float angle) const
{
	SDL_Point center{};
	center.x = dst.w / 2;
	center.y = dst.h / 2;
...
void dae::Renderer::RenderTexture(const Texture2D& texture, const float x, const float y , const SDL_Rect& src, float angle) const
{
	SDL_Rect dst{};
	dst.x = static_cast<int>(x);
	dst.y = static_cast<int>(y);
	dst.w = src.w;
	dst.h = src.h;
	RenderTexture(texture, dst, src, angle);
}
...
{
	return m_window ? SDL_GetWindowSurface(m_window)->w : 0;
}

int dae::Renderer::GetWindowHeight() const
{
	return m_window ? SDL_GetWindowSurface(m_window)->h : 0;
}
